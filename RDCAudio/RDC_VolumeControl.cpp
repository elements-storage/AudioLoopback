...
RDC_VolumeControl::RDC_VolumeControl(AudioObjectID inObjectID,
...
bool    RDC_VolumeControl::HasProperty(AudioObjectID inObjectID,
                                       pid_t inClientPID,
...
bool    RDC_VolumeControl::IsPropertySettable(AudioObjectID inObjectID,
                                              pid_t inClientPID,
                                              const AudioObjectPropertyAddress& inAddress) const
{
    CheckObjectID(inObjectID);
...
UInt32  RDC_VolumeControl::GetPropertyDataSize(AudioObjectID inObjectID,
                                               pid_t inClientPID,
...
void    RDC_VolumeControl::GetPropertyData(AudioObjectID inObjectID,
                                           pid_t inClientPID,
                                           const AudioObjectPropertyAddress& inAddress,
                                           UInt32 inQualifierDataSize,
    ...
    switch(inAddress.mSelector)
    {
        case kAudioLevelControlPropertyScalarValue:
            // This returns the value of the control in the normalized range of 0 to 1.
            {
                ThrowIf(inDataSize < sizeof(Float32),
                        ...
                        "RDC_VolumeControl::GetPropertyData: not enough space for the return value "
        ...
        case kAudioLevelControlPropertyDecibelRange:
            ...
            ThrowIf(inDataSize < sizeof(AudioValueRange),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "RDC_VolumeControl::GetPropertyData: not enough space for the return value of "
...
void    RDC_VolumeControl::SetPropertyData(AudioObjectID inObjectID,
                                           pid_t inClientPID,
                                           const AudioObjectPropertyAddress& inAddress,
                                           UInt32 inQualifierDataSize,
                                           const void* inQualifierData,
        ...
        case kAudioLevelControlPropertyScalarValue:
            {
                ThrowIf(inDataSize != sizeof(Float32),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "RDC_VolumeControl::SetPropertyData: wrong size for the data for "
...
void    RDC_VolumeControl::SetVolumeScalar(Float32 inNewVolumeScalar)
{
...
void    RDC_VolumeControl::SetVolumeDb(Float32 inNewVolumeDb)
{
...
void    RDC_VolumeControl::SetWillApplyVolumeToAudio(bool inWillApplyVolumeToAudio)
{
...
bool    RDC_VolumeControl::WillApplyVolumeToAudioRT() const
{
    return mWillApplyVolumeToAudio;
}

void    RDC_VolumeControl::ApplyVolumeToAudioRT(Float32* ioBuffer, UInt32 inBufferFrameSize) const
{
    ThrowIf(!mWillApplyVolumeToAudio,
            CAException(kAudioHardwareIllegalOperationError),
            "RDC_VolumeControl::ApplyVolumeToAudioRT: This control doesn't process audio data");
...
void    RDC_VolumeControl::SetVolumeRaw(SInt32 inNewVolumeRaw)
{
    CAMutex::Locker theLocker(mMutex);
    ...
    if(mVolumeRaw != inNewVolumeRaw)
    {
        ...
        RDCAssert((mAmplitudeGain >= 0.0f) && (mAmplitudeGain <= 1.0f), "Gain not in [0,1]");
        ...
        CADispatchQueue::GetGlobalSerialQueue().Dispatch(false, ^{
            ...
            RDC_PlugIn::Host_PropertiesChanged(GetObjectID(), 2, theChangedProperties);
        });
    }
}

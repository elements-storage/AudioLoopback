...
dispatch_queue_t RDCGetDispatchQueue_PriorityUserInteractive()
{
    long queueClass;
...
#if RDC_StopDebuggerOnLoggedExceptions || RDC_StopDebuggerOnLoggedUnexpectedExceptions
#if !RDC_StopDebuggerOnLoggedExceptions
            if(!expected)
#endif
            {
                RDCAssert(false, "CAException");
            }
#endif
            return e.GetError();
        }
...
#if RDC_StopDebuggerOnLoggedExceptions || RDC_StopDebuggerOnLoggedUnexpectedExceptions
            RDCAssert(false, "Unknown exception");
#endif
            return -1;
        }

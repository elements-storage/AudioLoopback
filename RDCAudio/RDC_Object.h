...
#ifndef __RDCDriver__RDC_Object__
#define __RDCDriver__RDC_Object__
...
//        been changed at all so far, except to replace "SA_Object" with "RDC_Object".

//==================================================================================================
//	RDC_Object
//
//	This is the base class for objects managed by RDC_ObjectMap. It's only job is to ensure that
...
//	that the reference count of the object is tracked and owned by the RDC_ObjectMap.
//
...
//
//	It is important that a thread retain and hold a reference while it is using an RDC_Object and 
//	that the reference be released promptly when the thread is finished using the object. By
//	assuming this, a RDC_Object can minimize the amount of locking it needs to do. In particular,
...
public:
						RDC_Object(AudioObjectID inObjectID, AudioClassID inClassID, AudioClassID inBaseClassID, AudioObjectID inOwnerObjectID);
...
private:
						RDC_Object(const RDC_Object&);
	RDC_Object&			operator=(const RDC_Object&);
...
#endif /* __RDCDriver__RDC_Object__ */

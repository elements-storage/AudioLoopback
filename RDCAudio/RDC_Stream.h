...
//
//  An input or output audio stream. Each stream belongs to a device (see RDC_AbstractDevice), which
//  in turn belongs to a plug-in (see RDC_PlugIn).
//
//  This class only handles the stream's HAL properties, i.e. the metadata about the stream, not the
//  audio data itself.
//

#ifndef RDCDriver__RDC_Stream
#define RDCDriver__RDC_Stream
...
public:
                                RDC_Stream(AudioObjectID inObjectID,

...
pthread_once_t     RDC_NullDevice::sStaticInitializer = PTHREAD_ONCE_INIT;
RDC_NullDevice*    RDC_NullDevice::sInstance = nullptr;

RDC_NullDevice&    RDC_NullDevice::GetInstance()
{
    pthread_once(&sStaticInitializer, StaticInitializer);
    return *sInstance;
}

void    RDC_NullDevice::StaticInitializer()
{
    try
    {
        sInstance = new RDC_NullDevice;
        // Note that we leave the device inactive initially. RDCApp will activate it when needed.
    }
    catch(...)
    {
        DebugMsg("RDC_NullDevice::StaticInitializer: Failed to create the device");
        delete sInstance;
        sInstance = nullptr;
    }
}

RDC_NullDevice::RDC_NullDevice()
:
    RDC_AbstractDevice(kObjectID_Device_Null, kAudioObjectPlugInObject),
...
void    RDC_NullDevice::Deactivate()
{
    CAMutex::Locker theStateLocker(mStateMutex);
...
void    RDC_NullDevice::SendDeviceIsAlivePropertyNotifications()
{
    CADispatchQueue::GetGlobalSerialQueue().Dispatch(false, ^{
        ...
        RDC_PlugIn::Host_PropertiesChanged(GetObjectID(), 1, theChangedProperties);
    });
}

#pragma mark Property Operations

bool    RDC_NullDevice::HasProperty(AudioObjectID inObjectID,
                                    pid_t inClientPID,
...
bool    RDC_NullDevice::IsPropertySettable(AudioObjectID inObjectID,
                                           pid_t inClientPID,
                                           const AudioObjectPropertyAddress& inAddress) const
{
...
UInt32    RDC_NullDevice::GetPropertyDataSize(AudioObjectID inObjectID,
                                            pid_t inClientPID,
                                            ...
                                            const void* __nullable inQualifierData) const
{
    ...
    switch(inAddress.mSelector)
    {
        ...
        default:
            theAnswer = RDC_AbstractDevice::GetPropertyDataSize(inObjectID,
                                                                inClientPID,
                                                                inAddress,
                                                                inQualifierDataSize,
                                                                inQualifierData);
...
void    RDC_NullDevice::GetPropertyData(AudioObjectID inObjectID,
                                        pid_t inClientPID,
                                        const AudioObjectPropertyAddress& inAddress,
                                        UInt32 inQualifierDataSize,
                                        ...
                                        void* outData) const
{
    ...
    // See RDC_Device::Device_GetPropertyData for more information about these properties.
    ...
    switch(inAddress.mSelector)
    {
        case kAudioObjectPropertyName:
            ThrowIf(inDataSize < sizeof(AudioObjectID),
                    ...
                    "RDC_NullDevice::GetPropertyData: not enough space for the return value of "
        ...
        case kAudioDevicePropertyDeviceUID:
            ...
            *reinterpret_cast<CFStringRef*>(outData) = CFSTR(kRDCNullDeviceUID);
            outDataSize = sizeof(CFStringRef);
            break;
        ...
        case kAudioDevicePropertyModelUID:
            ...
            *reinterpret_cast<CFStringRef*>(outData) = CFSTR(kRDCNullDeviceModelUID);
            outDataSize = sizeof(CFStringRef);
            break;
        ...
        default:
            RDC_AbstractDevice::GetPropertyData(inObjectID,
                                                inClientPID,
                                                inAddress,
                                                inQualifierDataSize,
                                                inQualifierData,
...
void    RDC_NullDevice::SetPropertyData(AudioObjectID inObjectID,
                                        pid_t inClientPID,
                                        const AudioObjectPropertyAddress& inAddress,
                                        UInt32 inQualifierDataSize,
                                        ...
                                        const void* inData)
{
    ...
    {
        RDC_AbstractDevice::SetPropertyData(inObjectID,
                                            inClientPID,
                                            inAddress,
                                            inQualifierDataSize,
                                            inQualifierData,
...
void    RDC_NullDevice::StartIO(UInt32 inClientID)
{
    ...
    if(mClientsDoingIO == 0)
    {
        ...
        // Send notifications.
        DebugMsg("RDC_NullDevice::StartIO: Sending kAudioDevicePropertyDeviceIsRunning");
        CADispatchQueue::GetGlobalSerialQueue().Dispatch(false, ^{
            AudioObjectPropertyAddress theChangedProperty[] = {
                CAPropertyAddress(kAudioDevicePropertyDeviceIsRunning)
            };
            RDC_PlugIn::Host_PropertiesChanged(kObjectID_Device_Null, 1, theChangedProperty);
        });
    }
...
void    RDC_NullDevice::StopIO(UInt32 inClientID)
{
    #pragma unused (inClientID)
    ...
    ThrowIf(mClientsDoingIO == 0,
            ...
            "RDC_NullDevice::StopIO: Underflowed mClientsDoingIO");
    ...
    {
        // Send notifications.
        DebugMsg("RDC_NullDevice::StopIO: Sending kAudioDevicePropertyDeviceIsRunning");
        CADispatchQueue::GetGlobalSerialQueue().Dispatch(false, ^{
            AudioObjectPropertyAddress theChangedProperty[] = {
                CAPropertyAddress(kAudioDevicePropertyDeviceIsRunning)
            };
...
void    RDC_NullDevice::GetZeroTimeStamp(Float64& outSampleTime,
                                         UInt64& outHostTime,
                                         UInt64& outSeed)
{
    CAMutex::Locker theIOLocker(mIOMutex);
    ...
    // clock similar to the loopback clock in RDC_Device.
...
void    RDC_NullDevice::WillDoIOOperation(UInt32 inOperationID,
                                          bool& outWillDo,
...
void    RDC_NullDevice::DoIOOperation(AudioObjectID inStreamObjectID,
                                      UInt32 inClientID,

...
RDC_Stream::RDC_Stream(AudioObjectID inObjectID,
                       AudioDeviceID inOwnerDeviceID,
                       bool inIsInput,
                       Float64 inSampleRate,
                       UInt32 inStartingChannel)
:
    RDC_Object(inObjectID, kAudioStreamClassID, kAudioObjectClassID, inOwnerDeviceID),
...
RDC_Stream::~RDC_Stream()
{
}

bool    RDC_Stream::HasProperty(AudioObjectID inObjectID,
                                pid_t inClientPID,
...
bool    RDC_Stream::IsPropertySettable(AudioObjectID inObjectID,
                                       pid_t inClientPID,
                                       const AudioObjectPropertyAddress& inAddress) const
{
    // For each object, this driver implements all the required properties plus a few extras that
...
UInt32    RDC_Stream::GetPropertyDataSize(AudioObjectID inObjectID,
                                          pid_t inClientPID,
...
void    RDC_Stream::GetPropertyData(AudioObjectID inObjectID,
                                    pid_t inClientPID,
                                    const AudioObjectPropertyAddress& inAddress,
                                    UInt32 inQualifierDataSize,
    ...
    switch(inAddress.mSelector)
    {
        case kAudioObjectPropertyBaseClass:
            ...
            ThrowIf(inDataSize < sizeof(AudioClassID),
                    ...
                    "RDC_Stream::GetPropertyData: not enough space for the return "
        ...
        case kAudioStreamPropertyDirection:
            ...
            ThrowIf(inDataSize < sizeof(UInt32),
                    CAException(kAudioHardwareBadPropertySizeError),
                    "RDC_Stream::GetPropertyData: not enough space for the return value of "
...
void    RDC_Stream::SetPropertyData(AudioObjectID inObjectID,
                                    pid_t inClientPID,
                                    const AudioObjectPropertyAddress& inAddress,
                                    UInt32 inQualifierDataSize,
                                    const void* __nullable inQualifierData,
        ...
        case kAudioStreamPropertyIsActive:
            {
                ...
                ThrowIf(inDataSize != sizeof(UInt32),
                        CAException(kAudioHardwareBadPropertySizeError),
                        "RDC_Stream::SetPropertyData: wrong size for the data for "
                ...
                if(mIsStreamActive != theNewIsActive)
                {
                    ...
                    CADispatchQueue::GetGlobalSerialQueue().Dispatch(false,	^{
                        ...
                        RDC_PlugIn::Host_PropertiesChanged(inObjectID, 1, theProperty);
                    });
                }
            }
            break;
        ...
        case kAudioStreamPropertyPhysicalFormat:
            {
                ...
                ThrowIf(theNewFormat->mFormatID != kAudioFormatLinearPCM,
                        ...
                        "RDC_Stream::SetPropertyData: unsupported format ID for "
                        "kAudioStreamPropertyPhysicalFormat");
                ThrowIf(theNewFormat->mFormatFlags !=
                        ...
                        CAException(kAudioDeviceUnsupportedFormatError),
                        "RDC_Stream::SetPropertyData: unsupported format flags for "
                        "kAudioStreamPropertyPhysicalFormat");
                ThrowIf(theNewFormat->mBytesPerPacket != 8,
                        CAException(kAudioDeviceUnsupportedFormatError),
                        "RDC_Stream::SetPropertyData: unsupported bytes per packet for "
                        "kAudioStreamPropertyPhysicalFormat");
                ThrowIf(theNewFormat->mFramesPerPacket != 1,
                        CAException(kAudioDeviceUnsupportedFormatError),
                        "RDC_Stream::SetPropertyData: unsupported frames per packet for "
                        "kAudioStreamPropertyPhysicalFormat");
                ThrowIf(theNewFormat->mBytesPerFrame != 8,
                        CAException(kAudioDeviceUnsupportedFormatError),
                        "RDC_Stream::SetPropertyData: unsupported bytes per frame for "
                        "kAudioStreamPropertyPhysicalFormat");
                ThrowIf(theNewFormat->mChannelsPerFrame != 2,
                        CAException(kAudioDeviceUnsupportedFormatError),
                        "RDC_Stream::SetPropertyData: unsupported channels per frame for "
                        "kAudioStreamPropertyPhysicalFormat");
                ThrowIf(theNewFormat->mBitsPerChannel != 32,
                        CAException(kAudioDeviceUnsupportedFormatError),
                        "RDC_Stream::SetPropertyData: unsupported bits per channel for "
                        "kAudioStreamPropertyPhysicalFormat");
                ThrowIf(theNewFormat->mSampleRate < 1.0,
                        CAException(kAudioDeviceUnsupportedFormatError),
                        "RDC_Stream::SetPropertyData: unsupported sample rate for "
                        "kAudioStreamPropertyPhysicalFormat");
            }
            break;

        default:
            RDC_Object::SetPropertyData(inObjectID,
                                        inClientPID,
                                        inAddress,
                                        inQualifierDataSize,
                                        inQualifierData,
...
void    RDC_Stream::SetSampleRate(Float64 inSampleRate)
{
    CAMutex::Locker theStateLocker(mStateMutex);
    mSampleRate = inSampleRate;
}
